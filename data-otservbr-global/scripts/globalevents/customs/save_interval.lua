local function serverSave(interval)
	...
	saveServer()
	local message = "Server save complete."
	local messageSingle = "Server save complete."
	Webhook.send("Server save", message, WEBHOOK_COLOR_WARNING)
	if SAVE_INTERVAL_CONFIG_TIME > 1 then
		Game.broadcastMessage(string.format(message, SAVE_INTERVAL_CONFIG_TIME, SAVE_INTERVAL_TYPE), MESSAGE_STATUS)
		Spdlog.info(string.format(message, SAVE_INTERVAL_CONFIG_TIME, SAVE_INTERVAL_TYPE))
	else
		Game.broadcastMessage(string.format(messageSingle, SAVE_INTERVAL_CONFIG_TIME, SAVE_INTERVAL_TYPE), MESSAGE_STATUS)
...
function save.onTime(interval)
	...
	if configManager.getBoolean(configKeys.TOGGLE_SAVE_INTERVAL) then
		local message = "Server save in " .. (remainingTime / 1000) .. " seconds."
		Game.broadcastMessage(message, MESSAGE_STATUS)
		Spdlog.info(string.format(message, SAVE_INTERVAL_CONFIG_TIME, SAVE_INTERVAL_TYPE))
